"""Session auth for Gestiolibra's JSON API -- shim sobre libraauth.
...
misma API pública (SessionAuth + dependencias json_api_*, 401/403 JSON sin
redirect, para APIs JSON puras sin página de login server-rendered). La
diferencia real está en services/users.py y main.py — el UserRepository de
libraauth trabaja sobre SQLAlchemy y recibe el session_factory del producto,
así que la tabla `usuarios` vive ahora en la base propia de Gestiolibra y no
en `gestiolibra_libracore.db`.
"""
...
def build_session_auth(users: UserRepository) -> SessionAuth:
    return SessionAuth(
        dev_secret_fallback="gestiolibra-dev-secret-not-for-prod",

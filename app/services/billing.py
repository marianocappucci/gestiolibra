"""Facturacion/caja para Gestiolibra, compuesto sobre libracore.db.

libracore.db es sqlite3 crudo, sin capa de abstraccion -- una conexion
propia, separada del engine SQLAlchemy que usa LibraGenda/Gestiolibra para
el resto del dominio (ver DECISIONS.md). Gestiolibra es de instancia unica
por cliente (arquitectura silo, igual que Contalibra/Restolibra), asi que
hay una sola "empresa" ARCA -- una constante fija, no una tabla de
empresas.

Una sola factura por turno completado (mismo diseño que MedLibra, ver
DECISIONS.md de ese repo): la seña ya cobrada (si existe) y el saldo
restante se reconcilian como dos movimientos de caja separados -- cada
uno con su propio medio de pago -- pero apuntando a la MISMA factura. La
seña nunca genera su propia factura.
"""
import os
...
def configure(db_path: str) -> None:
    ...
    if db_caja.get_default_caja_id() is None:
        caja_id = db_caja.create_caja_config(
            "Caja Gestiolibra", "", list(db_caja.MEDIOS_PAGO_LABELS),
        )
...
def _split_iva(total: Decimal) -> tuple[Decimal, Decimal]:
    ...
    al 21% -- simplificacion documentada, ver DECISIONS.md."""
...
async def invoice_appointment(
    client: dict, service_price: Decimal, referencia: str,
    ...
    tipo = _tipo_comprobante(client.get("condicion_iva"))
    ...
    factura_id = db_facturas.create_factura(
        ...
        client.get("cuit") or "", client.get("name") or "",
        IVA_CODES.get(client.get("condicion_iva"), IVA_CODES["Consumidor Final"]),

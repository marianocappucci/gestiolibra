"""Users: Gestiolibra's own table (not part of LibraGenda's domain -- auth
...
bootstrap (Gestiolibra has no Alembic of its own yet -- see TASKS.md).
"""
import os
...
def ensure_default_admin(repo: UserRepository) -> None:
    ...
    without at least one admin, nobody could ever create the rest of the
    accounts through the (now role-gated) /users API. Same fail-closed
    posture as libracore.auth's SECRET_KEY resolution: no admin password
    configured means the app refuses to boot, unless ENV=development.
    """
    if repo.list():
        return
    username = os.environ.get("GESTIOLIBRA_ADMIN_USERNAME", "admin")
    password = os.environ.get("GESTIOLIBRA_ADMIN_PASSWORD", "")
    if not password:
        if os.environ.get("ENV", "production") != "development":
            raise RuntimeError(
                "GESTIOLIBRA_ADMIN_PASSWORD no esta seteado. No se levanta la "

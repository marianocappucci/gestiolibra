"""Horario comercial: default weekly hours at the branch level, on top of
LibraGenda's own per-resource Availability.
...
resources also have to fall inside them. This keeps every existing test
and appointment flow unaffected until a business actually configures its
hours. Not part of LibraGenda's domain: whether a *business* (as opposed
to an individual resource) has its own default hours is a Gestiolibra
concept -- MedLibra never asked for it and LibraGenda's Availability is
already scoped to a single resource.
"""

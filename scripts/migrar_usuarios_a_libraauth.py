...
Contexto: hasta el 2026-07-30 el auth de Gestiolibra venia de
`libracore.db.usuarios`, que escribe con sqlite3 crudo en una base separada
(`gestiolibra_libracore.db`, junto a facturacion/caja/ARCA). Ahora lo provee
`libraauth` sobre SQLAlchemy, contra la MISMA base que el dominio
(`gestiolibra.db`). Este script mueve las filas existentes.
...
`ensure_default_admin()` crea un admin con `GESTIOLIBRA_ADMIN_PASSWORD`. Al
...
def main() -> int:
    ...
    ap.add_argument("origen", help="base de libracore (gestiolibra_libracore.db)")
    ap.add_argument("destino", help="base del producto (gestiolibra.db)")

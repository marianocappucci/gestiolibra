#!/usr/bin/env bash
# Reset diario de la demo publica de Gestiolibra.
#
...
# 2026-08-10 habia una copia suelta en `/root/scripts-demo/reset_gestiolibra.sh`
...
# no se borra nada.
SEED_LOCAL=/tmp/seed-gestiolibra.py
git -C /root/gestiolibra fetch -q origin || { log "ABORTA: no se pudo hacer fetch de gestiolibra."; exit 5; }
git -C /root/gestiolibra show origin/develop:scripts/seed_demo.py > "$SEED_LOCAL" || { log "ABORTA: no esta scripts/seed_demo.py en origin/develop."; exit 6; }
...
URL_CORE=$(docker exec "$CONTENEDOR" printenv GESTIOLIBRA_LIBRACORE_DB_PATH 2>/dev/null || true)
...
import sqlite3
try:
    c = sqlite3.connect('/app/data/gestiolibra.db')
...
docker exec -i "$CONTENEDOR" sh -c '
  python3 /tmp/seed.py \
    --url https://demo.gestiolibra.com.ar \
    --usuario "${GESTIOLIBRA_ADMIN_USERNAME:-admin}" \
    --password "$GESTIOLIBRA_ADMIN_PASSWORD"
'

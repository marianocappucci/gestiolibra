...
# la medianoche es irrepresentable: el alta lo rechaza con 409 "el recurso no
# esta disponible en ese horario", por mas que la disponibilidad diga
# 00:00-23:59 los siete dias.
#
...
def test_dashboard_with_no_data_returns_zeros(admin_client: TestClient):
    ...
    assert body["clientes"]["total_activos"] == 0
...
def test_dashboard_counts_appointments_by_status_and_today(admin_client: TestClient):
    ...
    created = client.post("/appointments", json={
        ...
        "client_id": "client-1", "starts_at": starts_at_dt.strftime("%Y-%m-%dT%H:%M:%S"),
    })
...
def test_dashboard_counts_active_clients_and_new_in_period(admin_client: TestClient):
    client = admin_client
    client.post("/clients", json={"id": "client-1", "name": "Ana"})
    client.post("/clients", json={"id": "client-2", "name": "Beto", "active": False})

    date_from, date_to = _today_range()
    response = client.get(f"/dashboard?date_from={date_from}&date_to={date_to}")
    assert response.status_code == 200
    body = response.json()
    assert body["clientes"]["total_activos"] == 1  # Beto esta inactivo
    assert body["clientes"]["nuevos_en_periodo"] == 2  # ambos se dieron de alta hoy


def test_dashboard_counts_reminders_sent_in_period(admin_client: TestClient):
    client = _seeded_client(admin_client)
    alta = client.post("/clients", json={"id": "client-1", "name": "Ana"})
    ...
    turno = client.post("/appointments", json={
        "resource_id": "resource-1", "service_id": "service-1",
        "client_id": "client-1", "starts_at": TURNO_FUTURO,
    })

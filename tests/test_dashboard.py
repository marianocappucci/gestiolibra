...
def test_dashboard_counts_appointments_by_status_and_today(admin_client: TestClient):
    # A small offset can still cross midnight if the test runs right
    # around 23:59 UTC -- the query range below is derived from the
    # appointment's own date, not from a separate "today" assumption.
    client = _seeded_client(admin_client)
    client.post("/clients", json={"id": "client-1", "name": "Ana"})

    starts_at_dt = datetime.now(timezone.utc) + timedelta(minutes=10)

...
def test_gestiolibra_creates_and_confirms_appointment(seeded_client: TestClient):
...
def test_cancel_appointment_with_reason(seeded_client: TestClient):
    ...
    response = client.post(
        f"/appointments/{appointment_id}/cancel", json={"reason": "cliente no puede asistir"},
    )
    assert response.status_code == 200
    assert response.json() == {
        "id": appointment_id, "status": "cancelled", "reason": "cliente no puede asistir",
    }
...
def test_reschedule_rejects_conflicting_slot(seeded_client: TestClient):
    ...
    second = client.post("/appointments", json={
        "resource_id": "resource-1", "service_id": "service-1",
        "client_id": "client-1", "starts_at": "2026-07-20T14:00:00",
    })
...
def test_staff_can_manage_own_appointments_but_not_catalog(seeded_client: TestClient, staff_client: TestClient):
    # seeded_client is the admin session that set up branch/resource/service/client.

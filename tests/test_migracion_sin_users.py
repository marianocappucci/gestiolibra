"""La migración `0008_sin_users` tiene que correr en LOS DOS MUNDOS.
...
`drop_table` el mundo 2 se pone rojo y la revisión no avanza.

## Por qué no se levanta la cadena entera

`0002`…`0007` tienen FK contra tablas de **LibraGenda** (`branches`), que viven
en otro paquete y otra cadena de Alembic — el CI ya las corre en orden. Acá lo
que se ejercita es **esta revisión**, así que se usa la `0001` real (que sólo
crea `users`, sin dependencias) y después se **estampa** la anterior. Intentar
`upgrade` hasta `0007` en el test falla por `relation "branches" does not exist`,
que es un rojo sobre otra cosa.
"""
...
def _revision() -> str | None:
    ...
    with psycopg.connect(_cruda(TEST_DATABASE_URL), autocommit=True) as c:
        fila = c.execute(
            "SELECT version_num FROM alembic_version_gestiolibra"
        ).fetchone()
    return fila[0] if fila else None


def test_mundo_1_la_borra_cuando_existe(base_limpia):
    """La base que armaron las migraciones: `0001` la creó, `0008` la saca.

    Se corre la `0001` de verdad —no un `CREATE TABLE` a mano— y después se
    estampa la anterior, salteando `0002`…`0007`, que dependen de LibraGenda.
    """

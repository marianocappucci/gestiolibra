...
def test_business_settings_update_round_trip(admin_client: TestClient):
    updated = admin_client.put("/business", json={
        "business_name": "Peluqueria Ana", "currency": "USD",
    })
    assert updated.status_code == 200
    assert updated.json() == {"business_name": "Peluqueria Ana", "currency": "USD"}

    fetched = admin_client.get("/business")
    assert fetched.json() == {"business_name": "Peluqueria Ana", "currency": "USD"}

...
mismo, portado a Gestiolibra el mismo dia.

🔑 **Aca los modelos vienen de dos lados**: diez de LibraGenda (`ClientRow`, `AppointmentRow`, ...) y cinco propios. Los dos cuelgan
...
def test_la_lista_no_quedo_vacia(admin_client):
    ...
    vacie `AUDITABLES`. Quince entidades al 2026-08-19 --eran dieciseis hasta
    que este mismo guard encontro `AgendaPolicyRow`, que nombraba una clase
    que el `libragenda@v0.9.0` pineado no tiene--."""
    assert len(AUDITABLES) >= 15, AUDITABLES
...
def test_el_registro_cubre_las_dos_procedencias(admin_client):
    ...
    assert "BranchHoursRow" in vivos, "no se ven los modelos propios"

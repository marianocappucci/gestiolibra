...
def cliente(tmp_path_factory):
    ...
    if creado:
        (dist / "index.html").write_text(
            "<!doctype html><title>Gestiolibra</title>", encoding="utf-8"
        )
    ...
    previo = {v: os.environ.get(v) for v in
              ("ENV", "GESTIOLIBRA_ADMIN_USERNAME", "GESTIOLIBRA_ADMIN_PASSWORD",
               "SECRET_KEY", "DATA_DIR")}
    os.environ["ENV"] = "development"
    os.environ["GESTIOLIBRA_ADMIN_USERNAME"] = "admin"
    os.environ["GESTIOLIBRA_ADMIN_PASSWORD"] = "clave-de-prueba"

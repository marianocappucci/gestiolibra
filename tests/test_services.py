...
def test_service_duplicate_id_returns_409():
    ...
    response = client.post("/services", json={"id": "service-1", "name": "Otro", "duration_minutes": 15})
    assert response.status_code == 409


def test_service_rejects_zero_duration():
    client = _client()
    response = client.post("/services", json={"id": "service-1", "name": "Corte", "duration_minutes": 0})

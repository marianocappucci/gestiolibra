...
def test_client_crud_round_trip():
    client = _client()
...
def test_client_not_found_returns_404():
    client = _client()
    assert client.get("/clients/missing").status_code == 404
    assert client.put("/clients/missing", json={"name": "x"}).status_code == 404
    assert client.delete("/clients/missing").status_code == 404


def test_client_duplicate_id_returns_409():
    client = _client()
    client.post("/clients", json={"id": "client-1", "name": "Ana"})
    response = client.post("/clients", json={"id": "client-1", "name": "Otra"})

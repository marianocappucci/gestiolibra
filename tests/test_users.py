...
# lo propio de Gestiolibra: que la tupla `roles=("admin","staff")` con la que
# se armó el router acá es la correcta, la integración con el login/sesión de
...
def test_update_user_rejects_invalid_role_returns_422_not_500(admin_client: TestClient):
    ...
    response = admin_client.put(f"/users/{created['id']}", json={
        "name": "Empleada", "role": "owner", "active": True,
    })
...
def test_update_user_role_and_deactivate(admin_client: TestClient):
    ...
    updated = client.put(f"/users/{created['id']}", json={
        "name": "Empleada Senior", "role": "admin", "active": False,
    })
    assert updated.status_code == 200
    body = updated.json()
    assert body["id"] == created["id"]
    assert body["username"] == "staff-1"
    assert body["name"] == "Empleada Senior"
...
def test_deactivated_user_cannot_log_in(admin_client: TestClient):
    ...
    client.put(f"/users/{created['id']}", json={"name": "Empleada", "role": "staff", "active": False})

...
def test_resource_crud_round_trip(admin_client: TestClient):
    ...
    assert client.get("/resources/resource-1").json()["name"] == "Box 1"
    assert len(client.get("/resources").json()) == 1

    updated = client.put("/resources/resource-1", json={"name": "Box renombrado", "active": False})
    assert updated.status_code == 200
    assert updated.json()["name"] == "Box renombrado"

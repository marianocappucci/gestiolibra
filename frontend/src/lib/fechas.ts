 ...
 *
 * El producto tenia el formateo repartido: un `fecha()` inline dentro de
 * `configuracion/recursos.tsx` y, dos secciones mas arriba en el mismo archivo,
 * un `.slice(0, 16).replace('T', ' ')` que dejaba el ISO a la vista. La agenda
 * usa su propio calculo de hora de pared en `components/agenda/datos.ts`, que
 * es otra cosa: ahi la zona es un dato de la sucursal.
 */

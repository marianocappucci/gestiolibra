...
// pertenecen es el del calendario de la sucursal. Un turno de las 21:30 del
// lunes en Buenos Aires es `2026-07-21T00:30:00Z` — martes en UTC. Si la
// pantalla agrupa por el string crudo, ese turno aparece en la columna del
// martes y quien atiende no lo ve el lunes. Es el mismo defecto que se arregló
// en el backend (ADR-030), del otro lado del cable.
...
const OTRO_RECURSO = { id: 'box-2', name: 'Box 2', branch_id: 'centro', active: true }
...
const DE_NOCHE = {
  id: 't-noche', resource_id: 'box-1', service_id: 'corte', client_id: 'ana',
...
const DE_MANANA = {
  id: 't-manana', resource_id: 'box-1', service_id: 'corte', client_id: 'ana',
...
function servir(turnos: unknown[], extra: Record<string, unknown> = {}) {
  fetchMock.mockImplementation((url: string, init?: RequestInit) => {
    ...
    if (u.includes('/agenda?')) {
      // El endpoint es por recurso: sólo el box-1 tiene turnos.
      return Promise.resolve(json(u.includes('/resources/box-1/') ? turnos : []))
    }
    if (u.includes('/resources')) return Promise.resolve(json([RECURSO, OTRO_RECURSO]))
...
describe('la agenda como calendario', () => {
  ...
  it('🔴 un turno de la noche cae en el día de la SUCURSAL, no en el de UTC', async () => {
    servir([DE_NOCHE])
    montar()
  ...
  it('pide el rango entero de la semana, una llamada por recurso', async () => {
    servir([])
    montar()
    await waitFor(() => expect(document.querySelectorAll('[data-columna]')).toHaveLength(7))
    const deAgenda = pedidos.filter((p) => p.url.includes('/agenda?')).map((p) => p.url)
    expect(deAgenda).toEqual([
      `/resources/box-1/agenda?date_from=${LUNES}&date_to=2026-07-26`,
      `/resources/box-2/agenda?date_from=${LUNES}&date_to=2026-07-26`,
    ])
  })
  ...
  it('🔴 abrir un turno lo pone en la URL y muestra su detalle', async () => {
    ...
    expect(within(dialogo).getByText('Corte')).toBeInTheDocument()
    expect(within(dialogo).getByText('Box 1')).toBeInTheDocument()
    // 10:00 de la sucursal, no 13:00 de UTC.
  ...
  it('🔴 un turno cancelado no lleva el color de su recurso', async () => {
    // Con el color del recurso se lee como un turno vivo y la columna diría que
    // el box está ocupado a esa hora cuando está libre.
  ...
  it('🔴 el control — un turno vivo SÍ lleva el color de su recurso', async () => {
    servir([DE_MANANA])
    montar()
    const bloque = await screen.findByText('Ana Gómez')
  ...
  it('el alta manda el turno con la hora tal como se escribió', async () => {
    ...
    await waitFor(() => {
      ...
      expect(alta?.cuerpo).toEqual({
        resource_id: 'box-1', service_id: 'corte', client_id: 'ana',
        // Naive, sin huso: el backend lo interpreta como hora de pared de la
        // sucursal. Mandarlo con offset sería decidir acá algo que decide allá.
  ...
  it('sin recursos activos lo dice y manda a Configuración', async () => {
    ...
    montar()
    expect(await screen.findByText(/Configuración . Recursos/)).toBeInTheDocument()
  })
})

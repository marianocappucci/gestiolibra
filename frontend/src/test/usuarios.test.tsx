...
const USUARIOS = [
  ...
  { id: 'u2', username: 'staff-1', name: 'Empleada', role: 'staff', active: true, email: '' },
]
...
it('ofrece Eliminar en la fila ajena y lo oculta en la propia', async () => {
  render(<Usuarios />)
  await screen.findByText('Empleada')

  expect(screen.getByRole('button', { name: 'Eliminar Empleada' })).toBeInTheDocument()

...
SQLite lo aceptaba por tipado dinamico -- guarda texto en una columna INTEGER
sin decir nada --, asi que el defecto era invisible: medido en la demo el
2026-08-09, de **86 filas habia 48 de texto**, 36 enteras y 2 nulas.

Contra PostgreSQL no hay tipado dinamico: el INSERT muere con
`invalid input syntax for type integer`. Y como el log se escribe
...
revision = "0007_entidad_id_texto"
down_revision = "0006_actividad_log"
branch_labels = None
depends_on = None
...
def downgrade():
    ...
    > `invalid input syntax for type integer` -- o sea que el

...
Verificado el 2026-08-24: `gestiolibra_core` tiene la tabla, `gestiolibra` no.

Nadie la consulta: `app/services/users.py` es un shim de tres lineas sobre
...
## Por que hacia falta sacarla, y no sólo ignorarla
...
este producto. O sea que la cadena de migraciones y la realidad decian cosas
distintas, y eso bloqueaba estampar las instancias para poder meter las
migraciones en el deploy: cualquier revision futura que tocara `users` iba a
fallar contra una base donde no existe.
...
Sobre una instancia viva estampada en `0007`, **no existe** --- y un
...
Revision ID: 0008_sin_users
Revises: 0007_entidad_id_texto
...
revision = "0008_sin_users"
down_revision = "0007_entidad_id_texto"
branch_labels = None
depends_on = None

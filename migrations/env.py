"""Alembic environment for Gestiolibra's own tables (not LibraGenda's).

`UserRow` is registered on LibraGenda's shared declarative Base (see
app/services/users.py), so target_metadata is deliberately left as None
...
hand, same convention LibraGenda itself uses.
...
VERSION_TABLE = "alembic_version_gestiolibra"


def get_url():
    return url_de_instancia("gestiolibra") or context.config.get_main_option("sqlalchemy.url")
